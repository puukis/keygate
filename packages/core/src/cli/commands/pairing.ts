...
const CHANNELS: PairingChannel[] = ['discord', 'slack'];
...
async function runApprove(args: ParsedArgs): Promise<void> {
  ...
  if (!channel || !code) {
    throw new Error('Usage: keygate pairing approve <discord|slack> <code>');
  }
...
function printPairingHelp(): void {
  console.log(`Pairing commands:\n  keygate pairing approve <discord|slack> <code>\n  keygate pairing pending [discord|slack]`);
}
